import { Building2, DatabaseZap, FolderKanban, Layers2, LayoutList, Save, TextSearch, Undo2 } from 'lucide-react';
...
export default function SearchConfig() {
  ...
  const fetchMetadata = async (documentTypeId) => {
    const response = await fetch(`http://localhost:3000/metadata/type/${documentTypeId}`);
    if (!response.ok) {
      throw new Error('Network response was not ok');
    }
    return response.json();
  ...
  useEffect(() => {
    if (selectedService) {
      fetchDocumentTypes(selectedService.id).then(setDocumentTypes).catch(console.error);
    }
  }, [selectedService]);

  useEffect(() => {
    if (selectedDocumentType) {
      fetchMetadata(selectedDocumentType.id).then(setMetadata).catch(console.error);
    }
  ...
  const handleMetadataClick = (metaId) => {
    setClickedMetadata(prev => 
      prev.includes(metaId) ? prev.filter(id => id !== metaId) : [...prev, metaId]
    );
  };
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      const metadataResponse = await fetch('http://localhost:3000/search-config/save-config', {
        method: 'POST',
        headers: {
      ...
      const switchResponse = await fetch('http://localhost:3000/search-result/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      <div className="flex-1 ml-2 flex flex-col">
        ...
        <div className="flex-1 mt-24 container mx-auto p-8 bg-white shadow-xl rounded-lg max-w-full overflow-auto">
          ...
          <div className="flex w-full flex-col md:flex-row gap-8 mb-10">
            <div className="w-full md:w-1/2">
              ...
              {servicesLoading ? (
                ...
                <select 
                  ...
                  onChange={(e) => {
                    const service = localServicesData.flatMap(directory => directory.services)
            ...
            {selectedService && (
              <div className="w-full md:w-1/2">
                ...
                <select 
                  className="w-full p-3 border border-indigo-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition duration-200 bg-white"
                  onChange={(e) => setSelectedDocumentType(documentTypes.find(dt => dt.id === parseInt(e.target.value)))}
                >
              ...
              <div className="flex flex-col md:flex-row gap-12">
                <div className="bg-gray-50 p-4 rounded-lg w-full border border-indigo-300 max-h-64 overflow-y-auto shadow-sm">
                  <ul className="list-disc pl-5">
                    {metadata.map(meta => (
                      <li key={meta.id} className="flex w-full bg-gray-200 p-2 rounded-lg items-center mb-2">
                        <input 
                          type="checkbox" 
                          id={`meta-${meta.id}`}
                          ...
                          className="mr-2"
                        />
                        <label htmlFor={`meta-${meta.id}`} className="flex-1">{meta.cle}</label>
                ...
                <div className="bg-gray-100 p-4 rounded-lg w-full border border-indigo-300 max-h-64 overflow-y-auto shadow-sm">
                  <h3 className="text-xl font-semibold mb-2 text-gray-800">Métadonnées sélectionnées</h3>
                  {metadata.filter(meta => selectedMetadata.includes(meta.id)).map(meta => (
                    <div  
                      key={meta.id} 
                      className={`mb-2 p-2 flex items-center justify-start gap-2 text-gray-800 rounded ${metadataDisplayState[meta.id] ? 'bg-cyan-200' : 'bg-gray-300'}`}
                    >
                      <DatabaseZap className="text-indigo-800" size={20} />
                      {meta.cle}
                      {clickedMetadata.includes(meta.id) && (
                        <span className="ml-2 text-indigo-700 cursor-help" title="Les métadonnées cliquées seront affichées">
                          ℹ️
                        </span>
                      )}
                    </div>
          ...
          <div className="flex flex-col md:flex-row justify-between gap-16">
            <button onClick={handleBack} className="bg-gray-600 flex items-center gap-2 text-white px-8 py-3 rounded-lg hover:bg-gray-700 transition duration-300">
              <Undo2 className="text-white" size={20} />
              Retour
            </button>
            <button onClick={handleSaveConfig} className="bg-indigo-700 flex items-center gap-2 text-white px-8 py-3 rounded-lg hover:bg-indigo-800 transition duration-300">
