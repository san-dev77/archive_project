...
const ContentContainer = ({ children }) => (
  <div className=" mt-24 p-1 bg-gray-100 rounded-lg  shadow-none flex flex-col h-screen items-stretch mr-10 w-full ml-7">
    {children}
  </div>
);
...
const StyledBox2 = ({ children }) => (
  <div className="bg-gray-800 rounded-lg shadow-2xl p-3 mb-4">{children}</div>
);
    ...
    <CenteredModal open={open} onClose={onClose} maxWidth="lg">
      <div className="flex justify-between items-center mb-4 border-b border-gray-600 pb-3">
        <div className="flex items-center">
          <InfoOutlined className="text-[#00B7FF] mr-2" sx={{ fontSize: 24 }} />
                  ...
                  >
                    <td className="w-1/6 text-white border-b border-gray-600 p-3">
                      <div className="flex justify-center">
                        <DatabaseZap className="text-cyan-400" size={18} />
                      </div>
                    </td>
                          ...
                          {Object.entries(uniqueFilesByPiece).map(
                            ([pieceName, files], index) => (
                              ...
                              >
                                ...
                                >
                                  <Typography className="text-white flex items-start justify-start bg-gray-700 p-2 rounded-lg font-bold truncate">
                                    <FileChartColumn />
                                    {pieceName}
                                  </Typography>
                                </AccordionSummary>
                                  ...
                                  <table className="w-full">
                                    <tbody>
                                      {files.map((file, fileIndex) => (
                                        <tr key={fileIndex}>
                                          <td className="w-1/6">
                                            <div className="hover:bg-cyan-600 cursor-pointer text-gray-800 rounded-lg bg-cyan-500 p-2 flex justify-center">
                                              <FileUp
                ...
                documentLot?.map((lot, index) => {
                  ...
                  return uniqueFiles.map((file, fileIndex) => (
                    <tr key={`${index}-${fileIndex}`}>
                      ...
                      <td colSpan="2" className="w-5/6">
                        <p
                          className="text-gray-800 truncate hover:text-blue-500 cursor-pointer"
                          onClick={() => handleFileClick(file.fileUrl)}
                        >
                          {file.filePath}
                        </p>
  ...
  <CenteredModal open={open} onClose={onClose} maxWidth="lg">
    <div className="flex justify-between items-center mb-4 border-b border-gray-600 pb-3">
      <h3 className="text-xl font-bold text-white flex items-center">
        <FileChartColumn className="mr-2 text-cyan-400" size={24} />
        Aperçu du fichier
      </h3>
      <button
        onClick={onClose}
    ...
    <div className="flex justify-end mt-4">
      <button
        onClick={onClose}
        className="px-6 py-3 text-sm font-medium text-white bg-gradient-to-r from-red-500 to-red-700 hover:from-red-600 hover:to-red-800 rounded-lg transition-all duration-200 shadow-md flex items-center gap-2"
      >
        <CircleX size={16} />
        Fermer
      </button>
...
const CreateEditModal = ({
  ...
  <div className="w-full">
    <div className="flex justify-between items-center mb-8 border-b border-gray-600 pb-4">
      <div className="flex items-center">
        {isEditing ? (
          <div className="bg-white p-4 rounded-full mr-4 shadow-lg">
            <SquarePen size={28} className="text-black" />
          </div>
        ) : (
          <div className="bg-gradient-to-r from-cyan-500 to-blue-500 p-4 rounded-full mr-4 shadow-lg">
            <FolderPlusIcon size={28} className="text-white" />
          </div>
        )}
      ...
      <button
        onClick={onClose}
        className="text-gray-400 hover:text-gray-200 bg-gray-700 hover:bg-gray-600 rounded-full p-3 transition-all duration-200"
      >
        <CircleX size={20} />
      </button>
    </div>

    <div className="bg-gradient-to-b from-[#3a3a3a] to-[#2a2a2a] rounded-lg p-6 mb-8 shadow-inner border border-gray-700">
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {metadataKeys.map((meta) => (
            <div key={meta.field} className="form-group">
              <label className="block text-base font-medium text-gray-300 mb-3 flex items-center">
                <DatabaseZap className="mr-2 text-cyan-400" size={18} />
                {meta.headerName}
              </label>
              <div className="relative">
                <input
                  ...
                  className="w-full px-5 py-4 bg-[#1f1f1f] text-white border border-gray-600 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200 shadow-sm text-base"
    ...
    <div className="flex justify-end space-x-5 mt-6">
      <button
        type="button"
        onClick={onClose}
        className="px-8 py-4 text-base font-medium text-white bg-gradient-to-r from-gray-600 to-gray-700 hover:from-gray-700 hover:to-gray-800 rounded-lg transition-all duration-200 shadow-md flex items-center gap-3"
      >
        <CircleX size={18} />
        Annuler
      </button>
      <button
        ...
        } rounded-lg transition-all duration-200 shadow-md flex items-center gap-3`}
      >
        {loadingForm ? (
            ...
            <div className="bg-[#3a3a3a] rounded-lg p-4 mb-6">
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-2">
                  <button
                    className="btn bg-white hover:bg-gray-700 text-black hover:text-white px-4 py-2 rounded-lg flex items-center transition-colors duration-200"
