...
import { FileCopyOutlined, InfoOutlined, LinkOutlined } from '@mui/icons-material';
...
import { ArchiveRestore, ArchiveRestoreIcon, ArrowUpDown, Building2, ChevronRightCircleIcon, DatabaseZap, File, FilePlus, FileUp, FolderCheck, FolderCheckIcon, FolderPlus, FolderPlusIcon, Layers3, LayoutPanelTop, Link, Plus, RefreshCcw, ScanEye, Search, SquarePen, Trash2 } from 'lucide-react';
...
const FloatingButton = ({ children, onClick }) => (
  <button
    className="btn btn-primary text-white z-50"
    onClick={onClick}
  >
    {children}
  </button>
...
const CenteredModal = ({ open, onClose, children }) => (
  ...
  >
    <div className="modal-box bg-white shadow-lg rounded-lg p-3"
      style={{
...
const DetailsDialog = ({ open, onClose, document, metadataKeys, documentLot, refreshDocuments, handleFileClick }) => {
                    ...
                    return (
                      <tr key={index} className=''>
                        <td className='hover:bg-cyan-600 cursor-pointer w-full text-gray-800 mt-2 rounded-lg bg-cyan-500 flex items-center justify-center'>
                          <FileUp className='cursor-pointer' onClick={() => handleFileClick(file.fileUrl)} />
                        </td>
                        <td className='text-gray-800 w-full'>
                          {file.pieceName}
                        </td>
                        <td>
                          <span className='text-gray-800 cursor-pointer hover:text-blue-500' onClick={() => handleFileClick(file.fileUrl)}>
                            {file.filePath}
                          </span>
                        </td>
                      </tr>
                ...
                documentLot?.map((lot, index) => {
                  ...
                  return uniqueFiles.map((file, fileIndex) => (
                    <tr key={`${index}-${fileIndex}`}>
                      <td className='hover:bg-cyan-600 cursor-pointer w-full text-gray-800 mt-2 rounded-lg bg-cyan-500 flex items-center justify-center'>
                        <LayoutPanelTop className='cursor-pointer' color='white' onClick={() => handleFileClick(file.fileUrl)} />
                      </td>
                      <td className='text-gray-800 w-full hover:text-blue-500 cursor-pointer' onClick={() => handleFileClick(file.fileUrl)}>
                        {file.filePath}
                      </td>
                    </tr>
                  ));
...
const DocumentListShow = () => {
  ...
  useEffect(() => {
    if (selectedDocType) {
      const fetchMetadataAndDocuments = async () => {
        try {
          const response = await axios.get(`http://localhost:3000/metadata/type/${selectedDocType}`);
  ...
  const fetchDocumentLot = async () => {
    ...
    try {
      const docResponseLot = await axios.get(`http://localhost:3000/documents/type/lot/${selectedDocType}`);
  ...
  const refreshDocuments = async () => {
    ...
    try {
      const docResponse = await axios.get(`http://localhost:3000/documents/type/pieces/${selectedDocType}`);
      if (docResponse.data) {
  ...
  const handleServiceChange = (event) => {
    ...
    const serviceName = servicesData.flatMap(directory => directory.services)
  ...
  const handleDocTypeChange = (event) => {
    ...
    const docTypeName = docTypes.find(docType => docType.id === docTypeId)?.name || '';
  ...
  const formattedDocuments = documents
    .filter(doc => {
      ...
      return (
        ...
        metadataKeys.some(key => (doc.metadata[key.field] || '').toLowerCase().includes(term))
      );
    })
    .map(doc => {
  ...
  const handleOpenDetails = (doc) => {
    ...
    const documentWithLotFiles = {
      ...doc,
      files: documentLot.find(d => d.id === doc.id)?.files || []
    };
    ...
    <MainContainer  >
      ...
      <ContentContainer >
        ...
        <h4 className=" font-bold text-gray-800 mb-1 text-xl mt-10 text-left w-full">
          <ArchiveIcon sx={{fontSize:"32px"}} className="text-gray-800 mr-2 " />
        ...
        <StyledBox>
          <div className="flex justify-between mb-3 gap-2">
            <div className="form-control w-1/2">
              ...
              <select
                className="select select-bordered bg-gray-200 border-gray-300"
                value={selectedService}
                onChange={(e) => handleServiceChange(e)}
              >
                <option value="">Sélectionner un service</option>
                {servicesData.map(directory => (
                  <optgroup className='text-black' key={directory.directory_id} label={directory.nom_directory}>
        ...
        <CenteredModal open={createDialogOpen} onClose={handleCloseCreateDialog}>
