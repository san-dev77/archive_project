...
// Fonction pour obtenir ou créer l'ID de la caisse
async function getOrCreateCaisseId(connection, code_caisse, agence_id) {
  const [result] = await connection.query(
    "SELECT id FROM caisse WHERE code_caisse = ? AND agence_id = ?",
    [code_caisse, agence_id]
  );
  //pour créer un repertoire pour chaque agence
  const dirPath = path.join(
    __dirname,
  ...
  await createDirectory(dirPath, code_caisse); // Appel de la méthode avec le nom de l'agence
  await createNestedDirectory(dirPath2, date_global, date_global);

  if (result.length > 0) {
    logger.info(
      `La caisse avec le code ${code_caisse} existe déjà avec l'ID ${result[0].id}`
    );
    return result[0].id;
  } else {
    await connection.query(
      "INSERT INTO caisse (code_caisse, agence_id) VALUES (?, ?)",
      [code_caisse, agence_id]
    );
    //pour créer un repertoire pour chaque agence
    const dirPath = path.join(
    ...
    logger.info(
      `La caisse avec le code ${code_caisse} a été créée avec l'ID ${newResult[0].id}`
    );
    return newResult[0].id;
  }
}
    ...
    for (const batch of batches) {
      ...
      try {
        for (const row of batch) {
          ...
          agence_global = row["AGENCE"];
          const agence = row["AGENCE"];
          const nom_prenom_caissier = row["NOM_ET_PRENOM_DU_CAISSIER"];
          const code_caisse = row["CODE_CAISSE"];
          const type_de_journee = row["TYPE_DE_JOURNEE"];
          const code_definitif = row["CODE_DEFINITIF"];
          const date = row["DATE"];
          date_global = row["DATE"];
          ...
          const caisseID = await getOrCreateCaisseId(
            connection,
            code_caisse,
            agenceID
          );
          ...
          await connection.query(
            `
            INSERT INTO transaction_caisse (agence_id, document_type_id, dates, nom_prenom_caissier, code_caisse, code_definitif)
            VALUES (?, ?, ?, ?, ?, ?)`,
            [
              agenceID,
              typeJourneeID,
...
async function importData(data, dbUrl, lineCount) {
